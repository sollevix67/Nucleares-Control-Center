...
function renderGenerators(generators) {
  ...
  if (emergency.length) {
    $("emergency-generator-list").innerHTML = emergency.map(generator => `<div class="generator-row"><div class="generator-number">E${generator.id}</div><div class="generator-copy"><strong>Groupe de secours ${generator.id}</strong><small><span class="status-pill ${generator.installed ? 'ok' : ''}">${escapeHtml(generator.installation_status)}</span>${generator.installed ? ` · <span class="status-pill ${generator.status_class}">${escapeHtml(generator.status)}</span> · mode ${escapeHtml(generator.mode ?? '—')}${generator.maintenance ? ' · MAINTENANCE REQUISE' : ''}` : ''}</small></div>
